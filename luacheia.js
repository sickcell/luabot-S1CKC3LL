    ...
    try {
        ...
        Client.cmd.on('ytmp4', async (data) => {
            try {
                ...
                Client.sendFileFromUrl(data.from, `${ytm.link}`, `${ytm.title} - Download.mp4`, `Video sent @${data.sender.split('@')[0]}`, data.message)
            } catch {
        ...
        Client.cmd.on('ig', async (data) => {
            if(isLimit(data.sender)) return data.reply(mess.limit)
            if(data.body == "") return data.reply(`Send orders *${data.prefix}ig [ link ]*\nExample : ${data.prefix}ig https://www.instagram.com/p/CPQJFm_jP7V/?utm_source=ig_web_copy_link`)
            data.reply(mess.wait)
        ...
        Client.cmd.on('joox', async (data) => {
            try {
                ...
                data.reply('Sorry lagu not found')
            }
        })
        /*RANDOM*/
        ...
        Client.cmd.on('limit', async (data) => {
            ...
            if(limits <= 0) return data.reply("```" + `Your limit has run out` + "```")
            data.reply(`Hai @${data.sender.split('@')[0]} 👋🏻\n Your limit remaining ${limits || 30}\nThe daily limit is reset hours 00.00\nIf you want to get unlimited limit, please chat bot owner, type !owner`)
        })
        ...
        Client.cmd.on('tebakgambar', async (data) => {
			if(isLimit(data.sender)) return data.reply(mess.limit)
			if (global.tebakgambar[data.from] && global.tebakgambar[data.from].id) return data.reply("There are still ongoing issues")
            const getSoal = await axios.get(`${configs.apiUrl}/api/tebakgambar?apikey=${configs.zeksKey}`)
			ses = Date.now()
			send = await Client.sendFileFromUrl(data.from, getSoal.data.result.soal, "soal.jpg", "Answer time 30 seconds!", data.message)
			global.tebakgambar[data.from] = {jawaban: getSoal.data.result.jawaban, id: ses}
			await sleep(10000)
			if (global.tebakgambar[data.from].id != ses) return
			Client.reply(data.from,"20 seconds remaining", send)
			await sleep(10000)
			if (global.tebakgambar[data.from].id != ses) return
			Client.reply(data.from,"10 seconds left", send)
			await sleep(10000)
			if (global.tebakgambar[data.from].id != ses) return
			Client.reply(data.from, "Time has run out", send)
        ...
        Client.cmd.on('bc', async (data) => {
            ...
            list.forEach(async dataC => {
                if(mediaBuffer) Client.sendFileFromBase64(dataC.jid, mediaBuffer.toString('base64'), `bc.${ext}`, `*LUA BOT BROADCAST*\n\n${data.body} ${dataC.jid.endsWith('@g.us') ?'\n'+dataC.name+'*_' : ''}`)
        ...
        Client.cmd.on('foxnews', async (data) => {
            ...
            var teks = `*「 FOXNEWS 」*\n\n`
            for(let i = 0; i < ttt.length; i++) {
                teks += `*Title* : ${ttt[i].title}\n*Time* : ${ttt[i].time}\n*Information*: ${ttt[i].content}\n*Country*: ${ttt[i].country}\n*Link*: ${ttt[i].url}\n\n`
            }
            await Client.sendFileFromUrl(data.from, ttt[0].thumb, 'p.jpg', teks, data.message)
        })
        /*GROUP*/
        Client.cmd.on('afk', (data) => {
            ...
            Client.sendText(data.from, "```" + `${data.pushname} [@${data.sender.split('@')[0]}] currently AFK\n\nReason: ${data.body}\nTime: ${timesNow}` + "```")
        })
...
`
        ...
        Client.cmd.on('promote', async (data) => {
            ...
            if(!data.botIsAdmin) return data.reply(mess.botAdmin)
            if(data.mentionedJidList.length == 0) return data.reply(`Send orders *${data.prefix}${data.command} [ @tag ]*\nExample : ${data.prefix}${data.command} @0`)
            client.groupMakeAdmin(data.from, data.mentionedJidList).then(() => data.reply(`Order accepted, added @${data.mentionedJidList.join(' @').replace(/@s.whatsapp.net/g, '')} sebagai admin.`)).catch(() => data.reply('Fail!'))
        })
        Client.cmd.on('demote', async (data) => {
            ...
            if(!data.botIsAdmin) return data.reply(mess.botAdmin)
            if(data.mentionedJidList.length == 0) return data.reply(`Send orders *${data.prefix}${data.command} [ @tag ]*\nExample : ${data.prefix}${data.command} @0`)
            client.groupDemoteAdmin(data.from, data.mentionedJidList).then(() => data.reply(`Order received, delete admin @${data.mentionedJidList.join(' @').replace(/@s.whatsapp.net/g, '')}`)).catch(() => data.reply('Fail!'))
        })
        Client.cmd.on('kick', async (data) => {
            ...
            if(!data.botIsAdmin) return data.reply(mess.botAdmin)
            if(data.mentionedJidList.length == 0) return data.reply(`Send orders *${data.prefix}${data.command} [ @tag ]*\nExample : ${data.prefix}${data.command} @0`)
            data.mentionedJidList.forEach(async jid =>{ client.groupRemove(data.from, [jid]).then(x => data.reply(`Success kick @${jid.split('@')[0]}`)).catch(x => data.reply(`Fail kick @${jid.split('@')[0]}`)); await sleep(2000)})
        })
        Client.cmd.on('add', async (data) => {
            ...
            if(!data.isAdmin) return data.reply(mess.admin)
            if(!data.botIsAdmin) return data.reply(mess.botAdmin)
            if(data.body == "") return data.reply(`Send orders *${data.prefix}${data.command} [ nomor ]*\nExample : ${data.prefix}${data.command} 9177367xxxx`)
            args = data.args.map(mp => mp + "@s.whatsapp.net")
            client.groupAdd(data.from, args).then(() => data.reply(`Added successfully@${data.args.join(' @')}`)).catch(() => data.reply('Unable to invite'))
        })
        ...
        Client.cmd.on('*', async (data) => {
            ...
            switch(command.toLowerCase()) {
                ...
                case 'watercolour':
                    ...
                    p = data.body
                    text = p.split('|')
                    Client.sendFileFromUrl(from, `${configs.apiUrl}/api/${command}?apikey=${configs.zeksKey}&text1=${text[0]}&text2=${text[1]}`, 'p.jpg', `*Image created successfully!* @${data.sender.split('@')[0]}`, message)
                    } catch {
                        data.reply('error')
                    }
					break
                ...
                case 'playstore':
                    try {
                        ...
                        data.reply(`Sorry app ${data.body} not found`)
                    }
                    break
                case 'wiki':
                    try {
                        ...
                        te = `*Search results for* : ${data.body}\n\n*Result* : ${res.data.result.result}`
                        data.reply(te)
                    } catch {
                        data.reply(`Sorry wiki ${data.body} not found`)
                    }
                    break	
                case 'kbbi':
                    try {
                        ...
                        te = `*Search result dari* : ${data.body}\n\n*Result* : ${res.data.result}\n*Source* : ${res.data.source}`
                        data.reply(te)
                    } catch {
                ...
                case 'happymod':
                    try {
                        ...
                        data.reply(`Sorry app MOD ${data.body} not found`)
                    }
                    break
                case 'iguser':
                    try {
                        if(isLimit(data.sender)) return data.reply(mess.limit)
                        if(data.body == "") return data.reply(`Send orders *${data.prefix}iguser [ username ]*\nExample : ${data.prefix}iguser lua cheia 007`)
                        data.reply(mess.wait)
...
`
                ...
                case 'brainly':
                    try {
                        ...
                        data.reply(`Sorry answer not found`)
                    }
                    break
                case 'spotify':
                    try {
                        if(isLimit(data.sender)) return data.reply(mess.limit)
                        if(data.body == "") return data.reply(`Send orders *${data.prefix}spotify [ lagu ]*\nExample : ${data.prefix}spotify my friend best`)
                        data.reply(mess.wait)
                ...
                case 'gsmarena':
                    try {
                        ...
                        data.reply(`Sorry mobile phone ${data.body} not found`)
                    }
                    break
                case 'searchmusic':
                case 'searchmusik':
                ...
                case 'tts':
                    if(isLimit(data.sender)) return data.reply(mess.limit)
                    if(data.body == "") return data.reply(`Send orders *${data.prefix}${data.command} [ code|teks ]*\nExample : ${data.prefix}${data.command} en|hello world`)
                    p = data.body
                    text = p.split('|')
                ...
                case 'stickermenu':
                    ...
                    Client.sendRawWebpAsSticker(from, fs.readFileSync('./lib/temp/sticks.webp'), message).then(resData => Client.sendText(from, 'use this sticker to make a sticker by replying to an image/video with this sticker', {
                        quoted: resData
                    }))
                    Client.sendRawWebpAsSticker(from, fs.readFileSync('./lib/temp/open.webp'), message).then(resData => Client.sendText(from, 'use this sticker to open a group', {
                        quoted: resData
                    }))
        ...
        Client.handlerStick.on("*", async (datas) => {
            ...
            switch(idStick) {
			    ...
			    case '1.4129505721465047e+123':
                    ...
                    datas.reply(`The group has been opened by admin @${datas.sender.split('@')[0]}`)
				    break
			    case '1.3049292658533466e+123':
                    ...
                    datas.reply(`The group has been closed by admin @${datas.sender.split('@')[0]}`)
				    break
            }
        })
    } catch (e) {
