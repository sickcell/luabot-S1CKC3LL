...
/*ANIME*/
        ...
        Client.cmd.on('anime', async (data) => {
			try {
			if(isLimit(data.sender)) return data.reply(mess.limit)
            if(data.body == "") return data.reply(`Send orders *${data.prefix}anime [ query ]*\nExample : ${data.prefix}anime naruto`)
        ...
        Client.cmd.on('manga', async (data) => {
			try {
			if(isLimit(data.sender)) return data.reply(mess.limit)
            if(data.body == "") return data.reply(`Send orders *${data.prefix}manga [ query ]*\nExample : ${data.prefix}manga naruto`)
            data.reply(mess.wait)
        ...
        Client.cmd.on('chara', async (data) => {
			try {
			if(isLimit(data.sender)) return data.reply(mess.limit)
            if(data.body == "") return data.reply(`Send orders *${data.prefix}chara [ query ]*\nExample : ${data.prefix}manga naruto`)
            data.reply(mess.wait)
...
*/
        Client.cmd.on('ytmp4', async (data) => {
            try {
                ...
                if(Number(ytm.size.split(' MB')[0]) >= 90.00) return Client.sendFileFromUrl(data.from, `${ytm.thumb}`, 'thumb.jpg', `*Data Successfully Obtained!*\n\n*Title* : ${ytm.title}\n*Size* : ${ytm.size}\n*Quality* : ${ytm.quality}\n*Ext* : mp4\n*Link* : ${ytm.link}\n\n_For the duration of more than the limit is presented in the form of a link_`, data.message)
        ...
        Client.cmd.on('tiktok', async(data) => {
            if (isLimit(data.sender)) return data.reply(mess.limit) 
               ...
               Client.sendFileFromUrl(data.from, `${configs.apiUhy}/api/socialmedia/tiktoknowm?url=${data.body}&apikey=BryanRfly`, 'tiktok.mp4', `Video successfully getting @${data.sender.split('@')[0]}`,  data.message)
        })
        ...
        Client.cmd.on('play', async (data) => {
            try {
                ...
                if(Number(ytm.size.split(' MB')[0]) >= 90.00) return Client.sendFileFromUrl(data.from, `${ytm.thumb}`, 'thumb.jpg', `*Data Berhasil Didapatkan!*\n\n*Title* : ${ytm.title}\n*Ukuran* : ${ytm.size}\n*Kualitas* : ${ytm.quality}\n*Ext* : mp3\n*Source* : ${ytm.source}\n*Link* : ${ytm.link}\n\n_Untuk durasi lebih dari batas disajikan dalam bentuk link_`, data.message)
                Client.sendFileFromUrl(data.from, ytm.thumb, 'thumb.jpg', teks, data.message)
                Client.sendFileFromUrl(data.from, ytm.link, `${ytm.title} - Download.mp3`, ``, data.message)
            } catch {
                data.reply('Oops sorry the server is in error or maybe the apikey is invalid')
        ...
        Client.cmd.on('ig', async (data) => {
            if(isLimit(data.sender)) return data.reply(mess.limit)
            if(data.body == "") return data.reply(`Send orders *${data.prefix}ig [ link ]*\nExample : ${data.prefix}ig https://www.instagram.com/p/CJ8XKFmJ4al/?igshid=1acpcqo44kgkn`)
            data.reply(mess.wait)
        ...
        Client.cmd.on('igstory', async (data) => {
            try {
                if(isLimit(data.sender)) return data.reply(mess.limit)
                if(data.body == "") return data.reply(`Send orders *${data.prefix}igstory [ username ]*\nExample : ${data.prefix}igstory JZ MODS`)
                data.reply(mess.wait)
                ...
                data.reply('Username not found')
            }
        })
        Client.cmd.on('joox', async (data) => {
            try {
                if(isLimit(data.sender)) return data.reply(mess.limit)
                if(data.body == "") return data.reply(`Send orders *${data.prefix}joox [ lagu ]*\nExample : ${data.prefix}joox bad liar`)
                data.reply(mess.wait)
                ...
                data.reply('Sorry the song was not found')
            }
        })
        /*RANDOM*/
        Client.cmd.on('asupan', async (data) => {
            ...
            if(data.args[0].toLowerCase() == '1') {
              Client.sendFileFromUrl(data.from, `${configs.apiUhy}/api/asupan/asupanukhty?apikey=${configs.uhykey}`, 'ukhty.mp4', `Heres the reason`, data.message)
            } else if(data.args[0].toLowerCase() == '2') {
              Client.sendFileFromUrl(data.from, `${configs.apiUhy}/api/asupan/asupan?apikey=${configs.uhykey}`, 'santuy.mp4', `Heres the reason`, data.message)
            } else if(data.args[0].toLowerCase() == '3') {
              Client.sendFileFromUrl(data.from, `${configs.apiUhy}/api/asupan/asupan?apikey=${configs.uhykey}`, '+62.mp4',`Heres the reason`, data.message)
            } else if(data.args[0].toLowerCase() == '4')  {
              Client.sendFileFromUrl(data.from, `${configs.apiUhy}/api/asupan/asupanbocil?apikey=${configs.uhykey}`, 'bocil.mp4', `Heres the reason`, data.message)
            } else if(data.args[0].toLowerCase() == '5') {
              Client.sendFileFromUrl(data.from, `${configs.apiUhy}/api/asupan/asupanrikagusriani?apikey=${configs.uhykey}`, 'rika.mp4', `Heres the reason`, data.message)
            } else if(data.args[0].toLowerCase() == '6') {
              Client.sendFileFromUrl(data.from, `${configs.apiUhy}/api/asupan/asupanghea?apikey=${configs.uhykey}`, 'ghea.mp4', `Heres the reason`, data.message)
            } else if(data.args[0].toLowerCase() == '7') {
              Client.sendFileFromUrl(data.from, 'https://pencarikode.xyz/api/chika?apikey=pais', 'chika.mp4', `Heres the reason`, data.message) 
            } else if(data.args[0].toLowerCase() == '8') {
              Client.sendFileFromUrl(data.from, 'https://pencarikode.xyz/api/asupan?apikey=pais', 'random.mp4', 'heres', data.message)
            } else {
              		let po = client.prepareMessageFromContent(data.from, {
					"listMessage":{
                  "title": `ASUPAN MENU`,
                  "description": `Menu of his intake ${data.pushname}`,
        ...
        Client.cmd.on('randomimg', async (data) => {
            if (isLimit(data.sender)) return data.reply(mess.limit) 
            if(data.args[0].toLowerCase() == '1') {
              Client.sendFileFromUrl(data.from, `${configs.apiLol}/api/random/cecan?apikey=${configs.lolkey}`, 'cecan.jpeg', 'Here Ramdom Image', data.message)
            } else if(data.args[0].toLowerCase() == '2') {
              Client.sendFileFromUrl(data.from, `${configs.apiLol}/api/random/cogan?apikey=${configs.lolkey}`, 'cogan.jpeg', 'Here Ramdom Image', data.message)
            } else if(data.args[0].toLowerCase() == '3') {
              Client.sendFileFromUrl(data.from, `${configs.apiLol}/api/random/bts?apikey=${configs.lolkey}`, 'bts.jpeg','Here Ramdom Image', data.message)
            } else if(data.args[0].toLowerCase() == '4')  {
              Client.sendFileFromUrl(data.from, `${configs.apiLol}/api/random/exo?apikey=${configs.lolkey}`, 'exo.jpeg', 'Here Ramdom Image', data.message)
            } else if(data.args[0].toLowerCase() == '5') {
              Client.sendFileFromUrl(data.from, `${configs.apiLol}/api/random/estetic?apikey=${configs.lolkey}`, 'estetik.jpeg', 'Here Ramdom Image', data.message)
            } else if(data.args[0].toLowerCase() == '6') {
              Client.sendFileFromUrl(data.from, `${configs.apiLol}/api/random/blackpink?apikey=${configs.lolkey}`, 'bpink.jpeg', 'Here Ramdom Image', data.message)
            } else if(data.args[0].toLowerCase() == '7') {
              Client.sendFileFromUrl(data.from, `${configs.apiLol}/api/random/waifu?apikey=${configs.lolkey}`, 'waifu.jpeg', 'Here Ramdom Image', data.message) 
            } else if(data.args[0].toLowerCase() == '8') {
              Client.sendFileFromUrl(data.from, `${configs.apiLol}/api/random/husbu?apikey=${configs.lolkey}`, 'husbu.jpeg', 'Here Ramdom Image', data.message)
            } else if(data.args[0].toLowerCase() == '9') {
              Client.sendFileFromUrl(data.from, `${configs.apiLol}/api/random/loli?apikey=${configs.lolkey}`, 'loli.jpeg', 'Here Ramdom Image', data.message)
            } else if(data.args[0].toLowerCase() == '10') {
              Client.sendFileFromUrl(data.from, `${configs.apiLol}/api/random/neko?apikey=${configs.lolkey}`, 'neko.jpeg','Here Ramdom Image', data.message)
            } else if(data.args[0].toLowerCase() == '11')  {
              Client.sendFileFromUrl(data.from, `${configs.apiLol}/api/random/elf?apikey=${configs.lolkey}`, 'elf.jpeg', 'Here Ramdom Image', data.message)
            } else if(data.args[0].toLowerCase() == '12') {
              Client.sendFileFromUrl(data.from, `${configs.apiLol}/api/random/shota?apikey=${configs.lolkey}`, 'shota.jpeg', 'Here Ramdom Image', data.message)
            } else if(data.args[0].toLowerCase() == '13') {
              Client.sendFileFromUrl(data.from, `${configs.apiLol}/api/random/sadgirl?apikey=${configs.lolkey}`, 'sadgirl.jpeg', 'Here Ramdom Image', data.message)
            } else if(data.args[0].toLowerCase() == '14') {
              Client.sendFileFromUrl(data.from, `${configs.apiLol}/api/random/art?apikey=${configs.lolkey}`, 'fanart.jpeg', 'Here Ramdom Image', data.message) 
            } else if(data.args[0].toLowerCase() == '15') {
              Client.sendFileFromUrl(data.from, `${configs.apiLol}/api/random/wallnime?apikey=${configs.lolkey}`, 'wallnime.jpeg', 'Here Ramdom Image', data.message)
            } else {
              		let po = client.prepareMessageFromContent(data.from, {
					"listMessage":{
                  "title": "RANDOM IMAGE",
                  "description": `Random Image Demand menu from ${data.pushname}`,
                  "buttonText": "Click here",
                  "listType": "SINGLE_SELECT",
                  "sections": [
                     {
        ...
        Client.cmd.on('limit', async (data) => {
            ...
            if(dataUser[data.sender].premium) return data.reply(`Hai @${data.sender.split('@')[0]} 👋🏻\nYou are a premium user that has limitless unlimited access!`)
            limits = configs.maxLimit - dataUser[data.sender].limit
            if(limits <= 0) return data.reply("```" + `Your limit is up` + "```")
            data.reply(`Hai @${data.sender.split('@')[0]} 👋🏻\n Your limit is left ${limits || 30}\nLimit every day reset hours 00.00\nIf you want to get unlimited limit please chat bot bot type !owner`)
        })
        /*OWNER*/
        ...
        Client.cmd.on('block', async (data) => {
            if(!data.isOwner) return data.reply(mess.ownerOnly)
            if(data.mentionedJidList.length == 0) return data.reply(`Send orders *${data.prefix}${data.command} [ @tag ]*\nExample : ${data.prefix}${data.command} @0`)
        ...
        Client.cmd.on('addvn', async (data) => {
            ...
            if(data.body == "") return data.reply(`Send orders ${data.prefix}addvn [ nama ]\nExample ${data.command}addvn hai`)
            if(vn.includes(data.body)) return data.reply('VN names already exist, please use another name')
            ...
            data.reply(`Successfully add a vn ${data.body} from the database`)
        })
        Client.cmd.on('delvn', async (data) => {
            ...
            data.reply(`Successfully delete vn ${data.body} from the database`)
        })
        Client.cmd.on('listvn', async (data) => {
            let listvn = 'Type the name VN for downloading vn\n\n*List vn*:\n\n'
        ...
        Client.cmd.on('join', async (data) => {
            if(!data.isOwner) return data.reply(mess.ownerOnly)
            if(data.body == "") return data.reply(`Link his?`)
            Client.acceptInviteLink(data.body).then(() => data.reply('ok')).catch(() => data.reply('failed'))
        })
        Client.cmd.on('owner', async (data) => {
            Client.sendContact(data.from, { number: configs.ownerList[0].split('@')[0], name: 'LUA CHEIA💌' }, data.message)
        })
        ...
        Client.cmd.on('afk', (data) => {
            ...
            Client.sendText(data.from, "```" + `${data.pushname} [@${data.sender.split('@')[0]}] was AFK\n\nThe reason: ${data.body}\nTime: ${timesNow}` + "```")
        })
...
`
        ...
        Client.cmd.on('setgroupname', async (data) => {
            ...
            if(!data.isAdmin) return data.reply(mess.admin)
            if(!data.botIsAdmin) return data.reply(mess.botAdmin)
            if(data.body == "") return data.reply(`Send orders *${data.prefix}${data.command} [ text ]*\nExample : ${data.prefix}${data.command} LUA`)
            client.groupUpdateSubject(data.from, `${data.body}`)
            data.reply(`The group name has been replaced by admin @${data.sender.split('@')[0]}`)
        })
        Client.cmd.on('setgroupdesc', async (data) => {
            ...
            data.reply(`Description Group has been replaced by the admin @${data.sender.split('@')[0]}`)
        })
        Client.cmd.on('promote', async (data) => {
            ...
            if(!data.botIsAdmin) return data.reply(mess.botAdmin)
            if(data.mentionedJidList.length == 0) return data.reply(`Send orders *${data.prefix}${data.command} [ @tag ]*\nExample : ${data.prefix}${data.command} @0`)
            client.groupMakeAdmin(data.from, data.mentionedJidList).then(() => data.reply(`Received orders, add @${data.mentionedJidList.join(' @').replace(/@s.whatsapp.net/g, '')} sebagai admin.`)).catch(() => data.reply('Failed!'))
        })
        Client.cmd.on('demote', async (data) => {
            ...
            if(!data.botIsAdmin) return data.reply(mess.botAdmin)
            if(data.mentionedJidList.length == 0) return data.reply(`Send orders *${data.prefix}${data.command} [ @tag ]*\nExample : ${data.prefix}${data.command} @0`)
            client.groupDemoteAdmin(data.from, data.mentionedJidList).then(() => data.reply(`The order is received, delete the admin @${data.mentionedJidList.join(' @').replace(/@s.whatsapp.net/g, '')}`)).catch(() => data.reply('Failed!'))
        })
        Client.cmd.on('kick', async (data) => {
            ...
            if(!data.botIsAdmin) return data.reply(mess.botAdmin)
            if(data.mentionedJidList.length == 0) return data.reply(`Send orders *${data.prefix}${data.command} [ @tag ]*\nExample : ${data.prefix}${data.command} @0`)
            client.groupRemove(data.from, data.mentionedJidList).then(() => data.reply(`Successfully removeding @${data.mentionedJidList.join(' @').replace(/@s.whatsapp.net/g, '')}`)).catch(() => data.reply('Failed!'))
        })
        Client.cmd.on('add', async (data) => {
            ...
            if(!data.isAdmin) return data.reply(mess.admin)
            if(!data.botIsAdmin) return data.reply(mess.botAdmin)
            if(data.body == "") return data.reply(`Send orders *${data.prefix}${data.command} [ nomor ]*\nExample : ${data.prefix}${data.command} 917736716373`)
            args = data.args.map(mp => mp + "@s.whatsapp.net")
            client.groupAdd(data.from, args).then(() => data.reply(`Succeed in adding @${data.args.join(' @')}`)).catch(() => data.reply('Unable to invite'))
        })
        ...
        Client.cmd.on('missing', async (data) => {
            ...
            if(data.isQuotedImage || data.type == 'imageMessage') {
                ...
                Client.sendFileFromBase64(data.from, getAxios.data.toString('base64'), 'missing.jpg', '*Image successfully made!* ', data.message)
        ...
        Client.cmd.on('calender', async (data) => {
            ...
            if(data.isQuotedImage || data.type == 'imageMessage') {
                ...
                Client.sendFileFromBase64(data.from, getAxios.data.toString('base64'), 'p.jpg', '*Image successfully made!* ', data.message)
            } else if(data.mentionedJidList.length > 0) {
                ppUrl = await client.getProfilePicture(data.mentionedJidList[0])
                if(!ppUrl) return data.reply('Profile picture not found!')
        ...
        Client.cmd.on('removebg', async (data) => {
            ...
            if(data.isQuotedImage || data.type == 'imageMessage') {
                ...
                ppUrl = await client.getProfilePicture(data.mentionedJidList[0])
                if(!ppUrl) return data.reply('Profile picture not found!')
                Client.sendFileFromUrl(data.from, `${configs.apiUrl}/api/removebg?apikey=${configs.zeksKey}&image=${encodeURIComponent(ppUrl)}`, 'calender.jpg', '*Image successfully made!* ', data.message)
            } else data.reply(`Wrong format!, tag someone or reply image with ${data.prefix}calender`)
        ...
        Client.cmd.on('drawing', async (data) => {
            ...
            if(data.isQuotedImage || data.type == 'imageMessage') {
                ...
                ppUrl = await client.getProfilePicture(data.mentionedJidList[0])
                if(!ppUrl) return data.reply('Profile picture not found!')
                Client.sendFileFromUrl(data.from, `${configs.apiUrl}/api/draw-image?apikey=${configs.zeksKey}&image=${encodeURIComponent(ppUrl)}`, 'calender.jpg', '*Image successfully made!* ', data.message)
        ...
        Client.cmd.on('sketch', async (data) => {
            ...
            if(data.isQuotedImage || data.type == 'imageMessage') {
                ...
                ppUrl = await client.getProfilePicture(data.mentionedJidList[0])
                if(!ppUrl) return data.reply('Profile picture not found!')
                Client.sendFileFromUrl(data.from, `${configs.apiUrl}/api/sketch-image?apikey=${configs.zeksKey}&image=${encodeURIComponent(ppUrl)}`, 'calender.jpg', '*Image successfully made!* ', data.message)
            } else data.reply(`Wrong format!, tag someone or reply image with ${data.prefix}drawing`)
...
*/
                ...
                case 'swm':
                    if(isLimit(data.sender)) return data.reply(mess.limit)
                    if(type != 'videoMessage' && !isQuotedVideo && !isQuotedImage && type != 'imageMessage') return data.reply('Wrong format!')
                    if(data.body == "") return data.reply(`Send orders *${data.prefix}${data.command} [ pack|author ]*\nExample : ${data.prefix}${data.command} punya|elios`)
                    data.reply(mess.wait)
                ...
                case 'semoji':
                    try {
                        if(isLimit(data.sender)) return data.reply(mess.limit)
                        if(data.body == "") return data.reply(`Send orders *${data.prefix}${data.command} [ emoji ]*\nExample : ${data.prefix}${data.command} 😃`)
                ...
                case 'take':
                    ...
                    if(data.body == "") return data.reply(`Send orders *${data.prefix}${data.command} [ pack|author ]*\nExample : ${data.prefix}${data.command} lua|cheia🎀`)
                    data.reply(mess.wait)
                    p = data.body
                    text = p.split('|')
                ...
                case 'nulis':
                    try {
                    if(isLimit(data.sender)) return data.reply(mess.limit)
                    if(data.body == "") return data.reply(`Send orders *${data.prefix}${data.command} [ teks ]*\nExample : ${data.prefix}${data.command} lua cheia`)
                    data.reply(mess.wait)
                    Client.sendFileFromUrl(from, `${configs.apiUrl}/api/${command}?text=${data.body}&apikey=${configs.zeksKey}`, 'gambar.jpg', `*Image successfully made!* @${data.sender.split('@')[0]}`, message)
                    } catch {
                        data.reply('error')
                    }
                    break
                ...
                case 'watercolour':
                    try {
                    if(isLimit(data.sender)) return data.reply(mess.limit)
                    if(data.body == "") return data.reply(`Send orders *${data.prefix}${data.command} [ teks1|teks2 ]*\nExample : ${data.prefix}${data.command} lua|cheia`)
                    data.reply(mess.wait)
                    p = data.body
                    text = p.split('|')
                    Client.sendFileFromUrl(from, `${configs.apiUrl}/api/${command}?apikey=${configs.zeksKey}&text1=${text[0]}&text2=${text[1]}`, 'p.jpg', `*Image successfully made!* @${data.sender.split('@')[0]}`, message)
                    } catch {
                        data.reply('error')
                    }
					break
                ...
                case 'hartatahta':
                    ...
                    if(data.body == "") return data.reply(`Send orders *${data.prefix}${data.command} [ teks ]*\nExample : ${data.prefix}${data.command} lua`)
                    data.reply(mess.wait)
                    Client.sendFileFromUrl(from, `${configs.apiUrl}/api/hartatahta?text=${data.body}&apikey=${configs.zeksKey}`, 'harta.jpg', `*Image successfully made!* @${data.sender.split('@')[0]}`, message)
                ...
                case 'playstore':
                    try {
                        if(isLimit(data.sender)) return data.reply(mess.limit)
                        if(data.body == "") return data.reply(`Send orders *${data.prefix}playstore [ apk ]*\nExample : ${data.prefix}playstore pubg`)
                        data.reply(mess.wait)
                        ...
                        data.reply(`Sorry aplikasi ${data.body} not found`)
                    }
                    break
                case 'wiki':
                    try {
                        if(isLimit(data.sender)) return data.reply(mess.limit)
                        if(data.body == "") return data.reply(`Send orders *${data.prefix}wiki [ query ]*\nExample : ${data.prefix}wiki manusia`)
                        data.reply(mess.wait)
                        res = await axios.get(`${configs.apiUrl}/api/wiki?apikey=${configs.zeksKey}&q=${data.body}`)
                        te = `*Search results from* : ${data.body}\n\n*Result* : ${res.data.result.result}`
                        data.reply(te)
                    } catch {
                        data.reply(`Maaf wiki ${data.body} not found`)
                    }
                    break	
                case 'kbbi':
                    try {
                        if(isLimit(data.sender)) return data.reply(mess.limit)
                        if(data.body == "") return data.reply(`Send orders *${data.prefix}kbbi [ query ]*\nExample : ${data.prefix}kbbi manusia`)
                        data.reply(mess.wait)
                        res = await axios.get(`${configs.apiUrl}/api/kbbi?apikey=${configs.zeksKey}&q=${data.body}`)
                        te = `*Search results from* : ${data.body}\n\n*Result* : ${res.data.result}\n*Source* : ${res.data.source}`
                        data.reply(te)
                    } catch {
                        data.reply(`Sorry kbbi ${data.body} not found`)
                    }
                    break
                case 'film':
                    try {
                        if(isLimit(data.sender)) return data.reply(mess.limit)
                        if(data.body == "") return data.reply(`Send orders *${data.prefix}film [ film ]*\nExample : ${data.prefix}film doraemon`)
                        data.reply(mess.wait)
                        ...
                        data.reply(`Sorry film ${data.body} not found`)
                    }
                    break
                case 'happymod':
                    try {
                        if(isLimit(data.sender)) return data.reply(mess.limit)
                        if(data.body == "") return data.reply(`Send orders *${data.prefix}happymod [ apk ]*\nExample : ${data.prefix}happymod pubg`)
                        data.reply(mess.wait)
...
*/
                case 'shopee':
                    try {
                        if(isLimit(data.sender)) return data.reply(mess.limit)
                        if(data.body == "") return data.reply(`Send orders *${data.prefix}shopee [ query ]*\nExample : ${data.prefix}shopee sepatu`)
                        data.reply(mess.wait)
                        res = await axios.get(`${configs.apiUrl}/api/shopee?apikey=${configs.zeksKey}&q=${data.body}`)
                        ttt = res.data.data
                        var teks = `*「 SHOPEE 」*\n\n*Search result : ${data.body}*\n\n`
                        for(let i = 0; i < ttt.length; i++) {
                            teks += `*Name* : ${ttt[i].name}\n*Prices*: ${ttt[i].harga}\n*Sold* : ${ttt[i].terjual}\n*Location*: ${ttt[i].location}\n*Description*: ${ttt[i].desc}\n*Stock*: ${ttt[i].stock}\n*Information*: ${ttt[i].information}\n*Link*: ${ttt[i].url}\n\n`
                        }
                ...
                case 'igstalk':
                    try {
                        if(isLimit(data.sender)) return data.reply(mess.limit)
                        if(data.body == "") return data.reply(`Send orders *${data.prefix}igstalk [ query ]*\nExample : ${data.prefix}igstalk lua.cheia007`)
                        data.reply(mess.wait)
...
`
                ...
                case 'brainly':
                    try {
                        if(isLimit(data.sender)) return data.reply(mess.limit)
                        if(data.body == "") return data.reply(`Send orders *${data.prefix}brainly [ query ]*\nExample : ${data.prefix}brainly who invented the lamp`)
                        data.reply(mess.wait)
                        res = await axios.get(`${configs.apiUrl}/api/brainly?apikey=${configs.zeksKey}&q=${data.body}&count=3`)
                        for(let i = 0; i < res.data.data.length; i++) {
                            await Client.reply(from, `Question : ${res.data.data[i].question}\n\nAnswer : ${res.data.data[i].answer[0].text}`, message)
                        }
                    } catch {
                ...
                case 'spotify':
                    try {
                        if(isLimit(data.sender)) return data.reply(mess.limit)
                        if(data.body == "") return data.reply(`Send orders *${data.prefix}spotify [ lagu ]*\nExample : ${data.prefix}spotify melukis senja`)
                        data.reply(mess.wait)
	...
	try {
            ...
            break
                case 'gsmarena':
                    try {
                        if(isLimit(data.sender)) return data.reply(mess.limit)
                        if(data.body == "") return data.reply(`Send orders *${data.prefix}gsmarena [ hp ]*\nExample : ${data.prefix}gsmarena asus rog phone 3`)
                        data.reply(mess.wait)
                        ...
                        data.reply(`Sorry hp ${data.body} not found`)
                    }
                    break
                case 'searchmusic':
                case 'searchmusik':
                ...
                case 'wallpaper':
                    ...
                    Client.sendFileFromUrl(from, image.img_hd, 'p.jpg', `*Search result* : ${data.body}`, message)
                    } catch {
                        data.reply(`error`)
                    }
                    break
                case 'pinterest':
				    try{
                    if(isLimit(data.sender)) return data.reply(mess.limit)
                    if(data.body == "") return data.reply(`Send orders *${data.prefix}pinterest [ query ]*\nExample : ${data.prefix}pinterest kucing`)
                    data.reply(mess.wait)
                    res = await axios.get(`${configs.apiUrl}/api/pinimg?apikey=${configs.zeksKey}&q=${data.body}`)
                    n = res.data.data
                    image = n[Math.floor(Math.random() * n.length)]
                    Client.sendFileFromUrl(from, image, 'p.jpg', `*Search result* : ${data.body}`, message)
                    } catch {
                        data.reply(`error`)
                    }
                    break
                case 'googleimage':
				    try{
                    if(isLimit(data.sender)) return data.reply(mess.limit)
                    if(data.body == "") return data.reply(`Send orders *${data.prefix}googleimage [ query ]*\nExample : ${data.prefix}googleimage hi from India`)
                    data.reply(mess.wait)
                ...
                case 'jagokata':
                    if(isLimit(data.sender)) return data.reply(mess.limit)
                    if(data.body == "") return data.reply(`Send orders *${data.prefix}jagokata [ apk ]*\nExample : ${data.prefix}jagokata bersyukurlah`)
                    data.reply(mess.wait)
                    res = await axios.get(`${configs.apiUrl}/api/jagokata?apikey=${configs.zeksKey}&q=${data.body}`)
                    if(res.data.status == false) data.reply(res.data.message)
                    ttt = res.data.result
                    var teks = `*「 JAGOKATA 」*\n\n*Search result : ${data.body}*\n\n`
                    ttt.forEach(tt1 => teks += `*Say* : ${tt1.kata}\n*Author* : ${tt1.author}\n*Info*: ${tt1.author_info}\n*Link*: ${tt1.author_url}\n\n` )
                    await data.reply(teks)
                    break
                    /*PRIMBON*/
                case 'jodoh':
                ...
                case 'pasangan':
                    if(isLimit(data.sender)) return data.reply(mess.limit)
                    if(data.body == "") return data.reply(`Send orders *${data.prefix}${data.command} [ kamu|dia ]*\nExample : ${data.prefix}${data.command} lua|cheia`)
                    data.reply(mess.wait)
                    p = data.body
                    text = p.split('|')
                    ...
                    if(res.data.status == false) data.reply(res.data.message)
                    p = res.data.result
                    tek = `*Your name* : ${p.nama1}\n*Her name* : ${p.nama2}\n\n*Positive result* : ${p.positif}\n*Negative result* : ${p.negatif}`
                    Client.sendFileFromUrl(from, p.thumb, 'p.jpg', tek, message)
                    break
                case 'artinama':
                    if(isLimit(data.sender)) return data.reply(mess.limit)
                    if(data.body == "") return data.reply(`Send orders *${data.prefix}artinama [ nama ]*\nExample : ${data.prefix}artinama elios`)
                    data.reply(mess.wait)
                ...
                case 'artimimpi':
                    if(isLimit(data.sender)) return data.reply(mess.limit)
                    if(data.body == "") return data.reply(`Send orders *${data.prefix}artimimpi[ mimpi ]*\nExample : ${data.prefix}artimimpi ular`)
                    data.reply(mess.wait)
                ...
                case 'jadwalshalat':
                    if(isLimit(data.sender)) return data.reply(mess.limit)
                    if(data.body == "") return data.reply(`Send orders *${data.prefix}${data.command} [ teks ]*\nExample : ${data.prefix}${data.command} jakarta`)
                    data.reply(mess.wait)
                ...
                case 'jadwaltv':
                    if(isLimit(data.sender)) return data.reply(mess.limit)
                    if(data.body == "") return data.reply(`Send orders *${data.prefix}${data.command} [ teks ]*\nExample : ${data.prefix}${data.command} antv`)
                    data.reply(mess.wait)
                ...
                case 'stickermenu':
                case 'stickercmd':
                    Client.sendRawWebpAsSticker(from, fs.readFileSync('./lib/temp/menus.webp'), message).then(resData => Client.sendText(from, 'Use this sticker to display the menu!', {
                        quoted: resData
                    }))
                    Client.sendRawWebpAsSticker(from, fs.readFileSync('./lib/temp/sticks.webp'), message).then(resData => Client.sendText(from, 'Use this sticker to make a sticker by replying image / video with this sticker', {
                        quoted: resData
                    }))
                    Client.sendRawWebpAsSticker(from, fs.readFileSync('./lib/temp/open.webp'), message).then(resData => Client.sendText(from, 'Use this sticker to open the group', {
                        quoted: resData
                    }))
                    Client.sendRawWebpAsSticker(from, fs.readFileSync('./lib/temp/close.webp'), message).then(resData => Client.sendText(from, 'Use this sticker to close the group', {
                        quoted: resData
                    }))
                    break
                case 'tes':
        ...
        Client.handlerStick.on("*", async (datas) => {
            ...
            /*	Cara bikin stickercmd 
                ...
                -case 'idnya': Example ada dibawah
            	*/
            switch(idStick) {
                case '2.453746655066493e+123':
                const time3 = moment().tz('Asia/Kolkata').format('HH:mm:ss')
if(time3 < "23:59:00"){
var ucapanWaktu = 'Good night'
                                        }
if(time3 < "20:00:00"){
          ...
          sec.push({
            "rows": [
          	...
          	let po = client.prepareMessageFromContent(from, {
				  "listMessage":{
                  "title": `MENU 𝖑𝖚𝖆 𝖇𝖔𝖙`,
                  "description": `${ucapanWaktu} bro ${pushname}`,
                  "buttonText": "MENU!",
                  "listType": "SINGLE_SELECT",
                  "sections": sec}}, {}) 
            client.relayWAMessage(po, {waitForAck: true})
			    ...
			    case '1.4129505721465047e+123':
                    ...
                    datas.reply(`Group opened by admin @${datas.sender.split('@')[0]}`)
				    break
			    case '1.3049292658533466e+123':
                    ...
                    datas.reply(`Group has been closed by the admin @${datas.sender.split('@')[0]}`)
				    break
            }
        })
    } catch (e) {
