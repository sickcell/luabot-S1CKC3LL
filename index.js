...
const starts = async (sesName) => {
    try {
        ...
        client.on('CB:Presence', asd => {
            ...
            Client.sendText(asd.id, `@${asd.participant.split('@')[0]} activity detected!, Your afk status has been deleted`)
                }
        })
        ...
        client.on('new-msg', (message) => {
			...
			if (global.tebakgambar[from] && global.tebakgambar[from].id && global.tebakgambar[from].jawaban.toLowerCase() == body.toLowerCase()) Client.reply(from, `YES GUESS THE PICTURE SUCCESSFULLY ANSWERED BY @${sender.split("@")[0]}`, message).then(() => global.tebakgambar[from] = {}) 
            ...
            if (isGroup && dataGc[from].antitagall && !message.isAdmin && (message.mentionedJidList.length == message.groupMembers.length || message.mentionedJidList.length-1 == message.groupMembers.length)){
                ...
                client.groupRemove(from, [sender]).catch(() => Client.reply(from, `Make an admin bot so that you can use the antitagall feature`, message))
            }
			...
			if (isGroup && !message.isAdmin && dataGc[from].antilink && /chat\.whatsapp\.com/gi.test(body)){
				...
				dtclink.forEach(async l => {
					...
					if(checks.status == 200){
						...
						client.groupRemove(from, [sender]).catch(() => Client.reply(from, `Make an admin bot so that you can use the antilink feature`, message))
					}
				})
			}
			if (!dataUser[sender]){
            ...
            if(isGroup) {
                ...
                if(message.message.extendedTextMessage && message.message.extendedTextMessage.contextInfo && message.message.extendedTextMessage.contextInfo.mentionedJid) {
                    ...
                    jids.forEach(jid => {
                        ...
                        Client.reply(from, `@${jid.split('@')[0]} sedang afk\nReason: ${takeData.reason}\nTime: ${duration.days()} Day ${duration.hours()} hours ${duration.minutes()} Minute ${duration.seconds()} second`)
                    })
                }
            }
        })
